...
@dataclass
class MovingSubPopulation_min_emp(Population):
    base_population: Population
    name: str = "moving_min_employees"

    status_filters: list = field(default_factory=lambda: ['NORMAL', 'AKTIV'])
    employee_filters: list = field(default_factory=lambda: [1, 1000000]) # [min, max] number of employees - max set to 1 million to avoid upper bound
...
if __name__ == "__main__":
    ...
    pop = MovingSubPopulation_min_emp(global_population)
    pop.prepare()
