...
from src.data.types import Background, CompanyDocument, EncodedDocument
...
class MLM(Task):
    ...
    def encode_document(self, document: CompanyDocument) -> "MLMEncodedDocument":
    ...
    def cls_task(self, document: CompanyDocument):
        p = np.random.rand(1)
        if p <0.05:
