...
from .source.base import TokenSource
...
class CorpusVocabulary(Vocabulary):
    ...
    def vocab(self) -> pd.DataFrame:
        ...
        for source in self.corpus.sources:
            ...
            all_field_labels.extend(source.field_labels())

        unique_field_labels = list(set(all_field_labels))
    ...
    def token_counts(self, source: TokenSource) -> Dict[str, Dict[str, int]]:
        ...
        tokenized = self.corpus.tokenized_and_transformed(source) \
                                .merge(ids, left_index=True, right_on='CVR', how='inner').drop(columns=['CVR'])
        ...
        for field_ in source.field_labels():
            ...
            counts[field_] = tokenized.reset_index()[["CVR", field_]].drop_duplicates()[field_].value_counts() 
